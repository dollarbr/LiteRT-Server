...
- All builds (debug and release) are signed with the shared keystore committed at `signing/shared.keystore` (passwords hardcoded in `app/build.gradle.kts` — intentionally public, debug-key semantics). This keeps the APK signature stable across CI runs so releases install over each other as updates. Never regenerate this file — existing installs would stop accepting updates.
...
6. **hf/HuggingFaceApi.kt** is a minimal HuggingFace Hub client: `searchModels(HfSearchParams)` hits `https://huggingface.co/api/models?filter=litert-lm&full=true` (the `filter` param matches the library tag — `library=` is silently ignored by the Hub API; `full=true` adds sibling filenames but never sizes) with query/author/sort/direction/limit driven by `ui/ModelBrowserScreen.kt`. Filename filtering and size sorting are client-side: `MainActivity.prefetchFileSizes` fills `hfFileSizes` (modelId → `.litertlm` files with sizes) via background `modelDetail` calls, 6 concurrent; `modelDetail` fetches per-model file listings filtered to `.litertlm` files. Sends `Authorization: Bearer <token>` when an HF token is configured. `HfModel.isGated` flags gated repos (🔒 badge); download taps without access (no token or `HfHttpException` 401/403) open the model's HF page via `MainActivity.openModelPage`. Multi-variant repos go through `VariantPickerDialog`, which recommends a file using `data/DeviceSpecs.kt` + `ModelFit` (SoC-matched NPU build first, else largest generic build fitting RAM; heuristics unit-tested in ModelFitTest).
