...
6. **hf/HuggingFaceApi.kt** is a minimal HuggingFace Hub client: searches `https://huggingface.co/api/models?filter=litert-lm` (the `filter` param matches the library tag; `library=` is silently ignored by the Hub API) with optional `author` and `sort` (downloads/likes/lastModified) params driven by chips in `ui/ModelBrowserScreen.kt`, fetches per-model file listings, and filters to `.litertlm` files. Sends `Authorization: Bearer <token>` when an HF token is configured (required for gated repos like official `google/gemma-*`).
